# Hi, I'm Aarav Chaudhry 👋

Second-year Mechatronics Systems Engineering student at [Your University], passionate about robotics, embedded systems, and control theory.

## 🔧 Projects

### Hardware/Embedded Systems
- 🤖 **[4-DOF Robotic Arm](https://github.com/yourusername/robotic-arm-4dof)** *(In Progress)*
  - Servo-controlled robotic arm with inverse kinematics
  - Tech: Raspberry Pi Pico, MG996R servos, 3D printed components
  
- 📏 **[Ultrasonic Distance Meter](https://github.com/yourusername/distance-meter)** *(In Progress)*
  - Real-time distance measurement and OLED display
  - Tech: Raspberry Pi Pico, HC-SR04 sensor, SSD1306 OLED

### Software & Algorithms
- 🎮 **[Pokemon Adventure Game](https://github.com/Aarav-Chaudhry/pokemon-adventure-game)** (C++)
  - Text-based RPG with grid navigation, turn-based combat, and state management
  - 550+ lines demonstrating data structures and algorithm design

## 🛠️ Skills

**Languages:** C++, Python, MicroPython  
**Hardware:** Raspberry Pi Pico, Arduino, Circuit Design, Soldering, 3D Printing  
**Tools:** Git, GitHub, Fusion 360, VS Code, Thonny

## 📚 Currently Learning

Embedded systems programming, robotics kinematics, and control systems

## 📫 Contact

📧 aca393@sfu.ca
💼 [LinkedIn](your-linkedin-url)
```
